    ...
    def _case_from_row(self, row: pd.Series) -> DefaultCase:
        ...
        addresses=[
            Address(
                ...
                phoneNumber=row.get("NU_TELEFON"),
        ...
        for _ , row in df.iterrows():
            for column in df.columns:
                if pd.notna(row[column]):
                    logger.debug("Coluna %s: %s", column, row[column])
            case = self._case_from_row(row)
            try:
                case_out = TypeAdapter(DefaultCase).validate_python(case)
                case_out = case_out.model_dump_json(indent=2)
